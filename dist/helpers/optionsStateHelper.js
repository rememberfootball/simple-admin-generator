...
const fieldsWithOptions = blocks => flatten(blocks.map(container => container.filter(b => !!b.optionsProvider).map(b => b.field)));

const fieldsReferences = blocks => flatten(blocks.map(container => container.filter(b => !!b.optionsProvider).map(b => ({
  field: b.field,
  ref: b.optionsProvider
}))));
...
const fetchOptions = async (definition, block) => {
  ...
  const fieldsWithThisRef = flatten(definition.blocks.map(container => container.filter(b => b.optionsProvider && b.optionsProvider === block.optionsProvider).map(b => b.field)));
  return zipObject(fieldsWithThisRef, fill(new Array(fieldsWithThisRef.length), results));
};
